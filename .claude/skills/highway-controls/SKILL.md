...
Controls follows the Interactive Workflow UX Contract in the Highway Experience Standard. Put the
`Next Action` first and omit implementation details unless requested. Guided Control Collection
reports `Step`, `Category`, `Completed Categories`, `Current Category`, and `Current Activity`.
Review Workflow reports `Current Proposal`, `Proposal Position`, `Remaining Proposals`, and
`Current Activity`; NFR Proposal Review reports `Current Candidate`, `Candidate Position`,
`Remaining Candidates`, and `Current Activity`. Each prompt contains one unresolved question or
decision.

`User Exits` are `pause`, `cancel`, or `stop responding`; `Owner Outcomes` are `declined`,
`aborted`, or `blocked`. `Resume Applicability`: `Persisted owner evidence`. Controls preserves
ownership of control artifacts, proposals, catalogs, identifiers, and completion claims.
...
Use `readiness` for a read-only assessment of whether the Control baseline is usable. Controls own
the validity decision and readiness never starts NFR proposal generation.
...
At least one valid Control with a consistent baseline is `Complete`. No valid Controls is `Missing`;
a malformed or inconsistent baseline is `Blocked`. A blocked response has a non-empty reason;
every other response uses `Blocking Reason: None`. Readiness does not write files, versions, IDs, or
NFR proposals.
...
**A catalog** at `library/governance/controls.md` following the complete structure in
`.highway/library/templates/output/control-catalog.md`. Controls are managed through this skill
rather than by hand.
