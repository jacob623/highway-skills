...
12. Order findings by category priority, source artifact order, source field name, and identifier. Identical inputs produce identical bytes with no volatile metadata.
13. Validate the catalog against its template, mappings, artifacts, supported values, direct Clarification Paths, and status agreement before mutation.
14. Catalog bootstrap: if the catalog is absent, construct it in memory from `.highway/library/templates/output/clarification-catalog.md`, add the proposed row, and run the same validation as an existing catalog.
15. Insert or replace the matching row, then order catalog rows by Artifact Type and Artifact ID.
16. Update rereads revision before writing; a mismatch returns conflict.
17. Successful Update appends history and increments revision once; competing updates are never merged.
18. Retry conflicts at most 3 times; then abort.
19. Generate and Update write both artifacts only after all validations succeed; Inspect, Read, and Status write nothing.
20. Derive status by precedence: `blocked` for malformed records, `complete` for valid zero-open records, `in-progress` for valid open records, and `not-started` only when absent.
