# Implementation Plan: Clarification Guided Resolution Workflow

**Branch**: `071-clarification-guided-resolution` | **Date**: 2026-09-22 | **Spec**: [spec.md](spec.md)

**Input**: Feature specification from `/specs/071-clarification-guided-resolution/spec.md`

**Note**: This template is filled in by the `/speckit-plan` command; its definition describes the execution workflow.

## Summary

Extend `highway-clarify` from a passive findings registry into a deterministic guided-resolution
workflow. The canonical skill and shared clarification-record template will emit question,
consequence, recommended, alternative, custom, rationale, source-traceability, and informational
selection fields for every finding while preserving user-owned responses, source immutability,
existing finding states, and governance, architecture, recommendation, and ADR ownership. The
implementation remains Markdown contract content plus Bash contract probes; generated adapters and
catalogs are refreshed through the existing generators.
...
**Performance Goals**: Preserve bounded deterministic record generation and validation; no new
latency or throughput target is required

**Constraints**: Catalog-only artifact resolution; declared-source-only guidance; no semantic
similarity or external knowledge; no automatic resolution or decision approval; source bytes and
finding identity must remain stable; invalid or conflicting writes produce no partial output;
generated artifacts must remain synchronized

**Scale/Scope**: Four supported artifact types (`REQ`, `DISC`, `ADR`, `RA`); one guidance block per
finding; existing record, catalog, revision, privacy, and count limits remain unchanged
...
| Validation Gate | Existing Clarification contract test changes | PASS planned | Preserve current probes and add guidance, precedence, conflict, selection, and immutability probes |
...
|---|---|---|
| D1.5 | PASS planned | This plan records the applicable constitution gates for the skill and shared-template changes |
...
```text
specs/071-clarification-guided-resolution/
...
template, and focused contract test. Use the existing generators for distributed adapters and
catalogs, and validate all dependent emitted contracts. No application source tree, runtime
service, or new persistence layer is introduced.
